...
interface Agg {
  ...
  // depth DURING sustained (>=1.5s) episodes: how far beyond the offside line the loiterer
  // actually sits while sustained (Task AT: distinguishes shallow oscillation from real
  // loitering — a high event count with ~0 depth is oscillation at a moving line, not camping)
  sustainedDepthSum: number;
  sustainedDepthN: number;
  boxSum: number;
  boxFrames: number;
  passesTotal: number;
...
function runConfig(soft: number, hold: number): Agg {
  ...
  const a: Agg = {
    ...
    sustainedDepthSum: 0, sustainedDepthN: 0,
  ...
  for (let m = 0; m < matches; m++) {
    ...
    for (let i = 0; i < steps; i++) {
      ...
      let any = false;
      const lineX = offsideLineX(w, def);
      const asgn = atk === 0 ? 1 : -1;
      for (const p of w.players) {
        ...
        if (isOffsidePosition(w, atk, p.pos)) {
          ...
          // Task AT: 持続(>=1.5s)エピソード中のオフサイド深さ(ライン超過量, m)。浅い振動
          // (ライン付近でのオンオフ)なら ~0、真のロイタリング(ライン裏に居座り)なら大きい。
          if (s >= 1.5 - 0.049) {
            a.sustainedDepthSum += asgn * p.pos.x - asgn * lineX;
            a.sustainedDepthN++;
          }
        } else {
          streak.set(p.id, 0);
...
function report(label: string, a: Agg): void {
  ...
  console.log(
    `Depth DURING sustained ep.: ${(a.sustainedDepthSum / Math.max(1, a.sustainedDepthN)).toFixed(2)} m beyond line ` +
      `(n=${a.sustainedDepthN}; ~0 = shallow oscillation at a moving line, large = real loitering)`,
  );
