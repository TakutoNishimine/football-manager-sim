...
function checkC(): CheckCResult {
  ...
  // 6. Goal kick that fires the Law-16 box-clearing branch (Task AB). CHECK C
  // case 4 only exercises the OVER-BAR generic restart (its lastTouchTeam=1 + +x
  // exit gives sign mismatch ⇒ isGoalKick=false); nothing in the probe pool hits
  // the true goal-kick branch, so its opponent x-displacement was untested. Here
  // team 1 puts the ball over team 0's OWN (−x) goal line above the bar ⇒ team-0
  // goal kick, with two team-1 opponents standing INSIDE team 0's box so the
  // insideBox clearing runs. The x-displacement must be sign-symmetric: team 0's
  // box clears toward −HALF_L+DEPTH+1, team 1's toward +HALF_L−DEPTH−1 (= its
  // negation), y preserved. If the sign were wrong the reflected clear would not
  // match B and this case fails.
  checkCComparingTicks(
    'goalkick-box-clear',
    () => {
      const w = bareWorld(16);
      parkPlayersAway(w);
      // Two team-1 opponents inside team 0's penalty area (x∈[-52.5,-36], |y|<20.16),
      // asymmetric in y so the y-spread preservation is genuinely exercised.
      w.players[12].pos = { x: -45, y: 6 };
      w.players[13].pos = { x: -48, y: -9 };
      // Ball above the crossbar (z>2.44 ⇒ not a goal) crossing team 0's −x line;
      // team 1 last touched ⇒ team 0 goal kick.
      w.ball = {
        pos: { x: -52, y: 0.5 },
        vel: { x: -12, y: 0 },
        z: 3.2,
