...
export function runPairing(
  sheets: TacticSheet[],
  pairing: Pairing,
  matchCount: number,
  minutes: number,
  seedBase: number = SEED_BASE,
): GameRecord[] {
  ...
  for (let m = 0; m < matchCount; m++) {
    ...
    // seedBase を変えると、同じ pairing/matchIndex でも異なるシードのマッチ集合になる
    // (Task AT: 色 A/B 用に「同じ N・異なるシード」の監査を回すため)。既定は SEED_BASE で不変。
    const seed = seedBase + pairing.pairingIndex * SEED_PAIRING_STRIDE + m;
