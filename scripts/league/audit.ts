...
import { buildPairings, runPairing, type GameRecord, type Pairing } from './audit-core.ts';
...
function parseArgs(argv: string[]): Args {
  ...
  if (!Number.isFinite(matches) || !Number.isInteger(matches) || matches <= 0) {
    throw new Error(`--matches must be a positive integer (got ${matches}); usage: [--matches N] [--minutes M] [--sheets a,b,c] [--quick] [--md] [--workers N]`);
  }
  return { matches, minutes, md, sheetFilter, workers };
}
...
async function main() {
  const { matches: matchCount, minutes, md, sheetFilter, workers } = parseArgs(process.argv.slice(2));
  ...
  if (workers === 1) {
    for (const pairing of pairings) {
      const records = runPairing(sheets, pairing, matchCount, minutes);
    ...
    await runPool<Pairing, GameRecord[]>({
      ...
      workerData: { sheets, matchCount, minutes },
      jobs: pairings,
      workers,
