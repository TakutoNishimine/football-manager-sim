...
// league:contrast (N=20試合×10分, extreme-vs-default) の実測。数値はTask AJのpost-U/pre-W計測
// のままで、post-W(Task W)の再計測は未実施 — tasks/BACKLOG.md「小items」#6の宿題。
// lineHeightだけはTask ACのブロック圧縮後もSim VerifyでLIVE 5.27σと再確認済み(reports/task-ac.md)。
...
const LEVER_BADGE = {
  ...
  // 0.3→PPDA 7.04, 0.6→PPDA 7.23: 効果0.12σ・有意差0.37σで中間域はほぼ不感帯。
  // 0付近/1付近の極端値でのみPPDAが動く(かつ強めるほどPPDAが悪化する逆転傾向あり、詳細はtask-aj.md)。
  pressIntensity:
    '<span class="lever-badge lever-badge--deadzone" title="0.3→PPDA 7.04 / 0.6→PPDA 7.23 (効果0.12σ・有意差0.37σ, task-aj, N=20×10分)。0.3〜0.6の中間域はほぼ不感帯で、0付近/1付近の極端値でしか効果が出ない。post-W再計測は未実施。">0.3〜0.6は不感帯</span>',
