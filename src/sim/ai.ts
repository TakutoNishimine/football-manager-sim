import {
  ...
  GK_HOLD_MIN,
  GK_HOLD_MAX,
  GK_HOLD_PRESSURE_CUT,
  GK_HOLD_STAND_DIST,
  GK_LONG_BASE,
  GK_LONG_PRESSURE_COEF,
  GOAL_WIDTH,
  HOLD_DURATION,
  HOLD_MAX_PRESSURE,
  ...
  SHOT_RANGE,
  SIM_DT,
  TAKEON_COVER_LOOKAHEAD,
  TAKEON_KNOCK_PAST,
  TAKEON_OPEN_BEHIND,
  TAKEON_RANGE,
} from './constants';
import { mulberry32Step, rand } from './rng';
...
export function laneRisk(
  ...
  // Task AF: 守備側の飛行反応半径は守備チームの pressIntensity の連続関数(press.ts 共有)
  const defTeam = (1 - attackingTeam) as 0 | 1;
  const reach = flightReachRadius(world.tactics[defTeam].pressIntensity);
  ...
  return laneRiskFromPoints(from, to, defenders, reach);
}
...
function loftedRisk(
  ...
  // Task AF: 守備側の飛行反応半径(press.ts 共有。laneRisk と同じグレーディング)
  const defTeam = (1 - owner.team) as 0 | 1;
  return loftedLaneRiskFromPoints(
    owner.pos,
    target,
    apexHeight,
    defenders,
    receiverArrival,
    flightReachRadius(world.tactics[defTeam].pressIntensity),
  );
}
...
/**
 * GK配球ホールド中の歩行目標(Task AB)。リリース地点 = 自ゴール前方 GK_BUILDUP_DEPTH の
 * ボックス端方向へゆっくり歩く。GK_BUILDUP_DEPTH(11m)< ボックス奥行き(16.5m)なので
 * ボックス内に収まり、前に出過ぎない。横は中央へ寄せる(実GKは配球前に中央へ持ち出して
 * 角度を作る)。移動速度は movePlayers 側で GK_WALK_SPEED に頭打ちする。
 */
function gkReleaseSpot(gk: Player): Vec {
  const sign = gk.team === 0 ? 1 : -1;
  const ownGoalX = -sign * HALF_L;
  const targetX = ownGoalX + sign * GK_BUILDUP_DEPTH;
  return clampToPitch(vec(targetX, gk.pos.y * 0.5), 2);
}
...
function takeOnCandidate(world: World, owner: Player): TakeOnCand | null {
  ...
  for (const o of opps) {
    ...
    // カバー距離は速度込みで読む(Task AF): 押し出しの徒競走(~1s)の間に走り込んでくる
    // カバー(2人目のプレッサー等)は、今は遠くても押し出す先を消しに来る。静止位置だけで
    // 見ると「向かってくるカバーへ仕掛ける」非現実な選択が過発火する(挟み込みの導入で顕在化)。
    // 攻撃側の意思決定なので predict ミラー対象外(Task W §4 と同じ前例)。
    const oAt = add(o.pos, scale(o.vel, TAKEON_COVER_LOOKAHEAD));
    openBehind = Math.min(openBehind, Math.min(dist(o.pos, beyond), dist(oAt, beyond)));
...
function decideOwner(world: World, owner: Player): void {
  ...
  if (owner.role === GK_ROLE) {
    ...
    // 配球の選択(Task AB): ショートの質(bestPassScore、クリーンなレーンで ~1.85)と、ロングの
    // 価値を競合オプションとして突き合わせる。単一のハードな圧力ゲート(旧: best && pressure<0.5)
    // ではない。ロング価値 = GK_LONG_BASE + GK_LONG_PRESSURE_COEF*pressure はプレッシャーで上がる
    // (詰められたら蹴り出す=既存パントの逃し弁を採点で表現)。クリーンなショートがあり無圧なら
    // ショートが勝つ=ほとんどの局面でショート。ショートが塞がれている(best 皆無 / bestPassScore が
    // 低い)か詰められているときパントが勝つ。圧力の即時脱出(相手が~2m=pressure>=CUT)は aiStep の
    // ホールド短縮(効果時間0)とこの採点(pressure↑でロング勝ち)の両輪で担保する。
    const shortValue = best !== null ? bestPassScore : -Infinity;
    const longValue = GK_LONG_BASE + GK_LONG_PRESSURE_COEF * pressure;
    if (best !== null && shortValue >= longValue) {
      executePass(world, owner, passTarget(world, owner, best));
    } else {
      // パント(Task AA): 圧力の逃し弁 / ショートが塞がれた時のロング。最前線の味方のサイドへ
      // 55〜70mの高いボールを送り、着地は素のレースで争う。
...
// プレス目標点・回復ラン目標・ジョッキー/カバーのジオメトリは press.ts の共有実装を使う
// (Task AF)。マーク目標点は line.ts の markTargetPoint(共有実装)を使う(Task AD)。
...
function decideDefense(world: World, defendersTeam: number): void {
  ...
  if (owner && buildup) {
    ...
    if (fws.length) {
      ...
      // Task AF: 出て行く距離は pressIntensity の連続関数(press.ts 共有。既定 0.5 で従来の 18m)
      if (dist(nearestFw.pos, ball.pos) < fwPressRange(tactics.pressIntensity)) fwPresser = nearestFw;
    }
  }
  ...
  if (!presser.instruction) {
    ...
    if (owner) {
      if (presser === fwPresser) {
        ...
        // Task AF: カバー(pressIntensity で連続に変わる半径内の味方)がいればリード点へ
        // コミット(W のタックル機構が引き継ぐ)、いなければジョッキーで外へ追い出す。
        // 旧 <0.3 コンテイン閾値は置換(不感帯の解消)。式は press.ts の共有実装(ミラー invariant)。
        target = hasPressCover(defenders, presser, owner, oppSign, tactics.pressIntensity)
          ? pressTarget(owner)
          : jockeyTarget(owner, ownGoal, tactics.pressIntensity);
      }
    } else {
  ...
  // 2人目のプレッサー(Task AF): 2番目にボールに近い守備者が secondPressRange(pressIntensity の
  // 連続関数。旧 >0.6 ハード閾値の置換)以内なら、保持者の逃げレーン上 4-6m ゴール側のカバー点へ
  // 送る(旧 pressTarget(owner) = 1人目と同一点への尾行 = ゼロカバー、を廃止)。
  ...
  if (owner) {
    ...
    if (rest.length) {
      const cand = rest.reduce((a, b) =>
        dist(a.pos, ball.pos) < dist(b.pos, ball.pos) ? a : b,
      );
      if (dist(cand.pos, ball.pos) < secondPressRange(tactics.pressIntensity)) {
        secondPresser = cand;
        if (!secondPresser.instruction)
          setIntent(secondPresser, 'press', pressCoverTarget(owner, oppSign, tactics.pressIntensity));
  ...
  for (const m of markers) {
    ...
    if (mark) {
      ...
      setIntent(m, 'mark', markTargetPoint(mark.pos, ownGoal, ball.pos, markOffset, ownLineX, defSign, pressAggroScale(tactics.pressIntensity)));
      continue;
    }
    ...
    // Task AF: 受諾半径は pressIntensity のコース狩り係数で連続に変わる(既定 0.5 で従来と一致。
    // press=0 の守備は持ち場で構えてコースを狩らない)。この分岐は task-q 以来 predict 未ミラー
    // (predict は cover 扱い)なので、半径の変更もその文書化済み非対称の内側に留まる。
    if (owner) {
      const laneReach = zoneRadius * pressAggroScale(tactics.pressIntensity);
      ...
      for (const t of targets) {
        ...
        if (dist(onLane, anchor) > laneReach) continue;
        const danger = -dist(t.pos, ownGoal);
...
// Task AV: tick 内の意思決定 ORDER。flip 実験(runs 29137051701〜29137054942)で、歴史的な
// 「team0 が毎tick先に決定」順そのものが恒常的な先手有利(決着試合の約51%、プール1.99σ)=
// 監査の color バイアスの機構だと確認された(lean の符号が順序の反転に完全追従)。
// 出荷既定 'alternate': firstTeam = (tickIndex + seedBit) & 1 の決定論的な毎tick交互。
// tickIndex = round(clock/SIM_DT) — clock は stepPhysics 末尾で加算されるので aiStep 時点の
// 値が tick 番号(UI もヘッドレスも固定 SIM_DT の累積器で呼ぶ)。seedBit は world.seed から
// mulberry32 で導出した混合1bit(Date/Math.random は使わない。world.rngState は消費しない。
// シード列に偶奇の構造があっても開始チームが偏らない)。期待値で両チームの先手回数が一致し、
// 先手有利チャネルが構造的に消える。チーム内の id 順は不変。
// 'team0First'(= main の歴史的順序とバイト一致)/ 'team1First' は診断・掃引用に残す
// (setShotErrScale / setTeamWeights と同型の in-process 設定。color-probe --order で使用)。
// world.players 自体は並べ替えない(tick 毎に反復配列を作る)。
export type DecisionOrder = 'alternate' | 'team0First' | 'team1First';
let DECISION_ORDER: DecisionOrder = 'alternate';
export function setDecisionOrder(order: DecisionOrder): void {
  DECISION_ORDER = order;
}

// symmetry-probe 専用のフェーズ反転フック(Task AV)。チーム交換鏡映 R は「team f が先に決定する」
// 状態を「team 1−f が先に決定する」状態へ写す — 交互フェーズはチーム添字付きの世界状態の一部で
// あり、R の下で他の per-team 配列と同様に写像されなければならない(健全な不変量は
// R(S_f(W)) = S_{1−f}(R(W)))。プローブは鏡映世界の aiStep をこのフラグ下で実行する。
// 'alternate' 以外では不使用(固定順はフェーズを持たず、プローブは固定順を id 整列のまま検査する)。
// シミュレーション本体は絶対に触らない診断専用ガード(setDecisionOrder と同型)。
let MIRROR_PHASE_FLIP = false;
export function setMirrorPhaseFlip(v: boolean): void {
  MIRROR_PHASE_FLIP = v;
}

/** この tick に先に意思決定するチーム(player loop とフリーボール decideDefense の両方に適用) */
function firstTeamThisTick(world: World): 0 | 1 {
  if (DECISION_ORDER === 'team0First') return 0;
  if (DECISION_ORDER === 'team1First') return 1;
  // alternate: clock の累積浮動小数誤差は10分(72,000 tick)で ~1e-8 << 0.5 なので round は正確
  const tick = Math.round(world.clock / SIM_DT);
  const seedBit = mulberry32Step((world.seed ^ 0x517cc1b7) >>> 0)[1] < 0.5 ? 0 : 1;
  const flip = MIRROR_PHASE_FLIP ? 1 : 0;
  return ((tick + seedBit + flip) & 1) as 0 | 1;
}
...
export function aiStep(world: World, dt: number): void {
  const owner = ballOwner(world);
  const possTeam = owner?.team ?? null;
  // Task AV: 意思決定順の反復配列。'team0First' は生の world.players を反復(= main の歴史的順序と
  // バイト一致 — このガードの存在理由)。alternate / team1First はチーム意味論で構築する:
  // firstTeam のチームの選手を先に、残りを後に(filter はチーム内の相対 id 順を保つ)。canonical な
  // world(ids 0-10 = team0)では [...t0, ...t1] は生配列と同一列だが、symmetry-probe のチーム
  // 再ラベル鏡映世界では配列先頭が team1 になるため、「配列順」と「team 0 が先」は別物 — フェーズ
  // 反転した鏡映で物理的な反復順が一致するのはチーム意味論の構築だけ(probe FAIL 29140984144 の教訓)。
  // world.players 自体は不変。turnover ワイプ(下)は全選手の独立リセットで順序非依存のため対象外。
  const firstTeam = firstTeamThisTick(world);
  let decisionOrder: readonly Player[];
  if (DECISION_ORDER === 'team0First') {
    decisionOrder = world.players;
  } else {
    const t0 = world.players.filter((p) => p.team === 0);
    const t1 = world.players.filter((p) => p.team === 1);
    decisionOrder = firstTeam === 0 ? [...t0, ...t1] : [...t1, ...t0];
  }
  ...
  for (const p of decisionOrder) {
    ...
    if (p.role === GK_ROLE && world.ball.ownerId !== p.id) {
      p.gkGainedAt = -1; // 手放した: 次の受球でホールドを再 seed(Task AB)
    ...
    // GK owner(Task AB): 受球後にホールドしてから配球する。zero hold(受けた瞬間に配球)を、
    // ~2〜5秒ためてから配るリズムに置換し、最も見られる局面(GKリセット)の lull-burst を戻す。
    // 上の keeper 分岐が off-ball で gkGainedAt を -1 に戻すので、ここに来た初回だけ seed する。
    // プレッシャーがホールドを滑らかに縮める: CALM 以下(相手~3.5m以遠)はフル、CALM→CUT で
    // 線形に0へ、CUT(相手~2.4m、TACKLE_RANGE の外側)で即配球=既存のパントの逃し弁を殺さず、
    // タックルが届く前に放す(§4 プレスファーム対策)。seed は world.rng のみ(決定論)。
    // GK は整え(settle)対象外なので下の isOwnerP 分岐には入れず、ここで完結させる。
    if (p.role === GK_ROLE && world.ball.ownerId === p.id) {
      if (p.gkGainedAt < 0) {
        p.gkGainedAt = world.clock;
        p.gkHoldBase = GK_HOLD_MIN + rand(world) * (GK_HOLD_MAX - GK_HOLD_MIN);
      }
      const pr = pressureOf(world, p);
      const holdFactor =
        1 - Math.max(0, Math.min(1, (pr - GK_HOLD_CALM) / (GK_HOLD_PRESSURE_CUT - GK_HOLD_CALM)));
      const effHold = p.gkHoldBase * holdFactor;
      if (world.clock - p.gkGainedAt < effHold) {
        // ホールド中: 無圧ならリリース地点(ボックス端方向)へゆっくり歩く(速度は GK_WALK_SPEED で
        // 頭打ち)。相手が GK_HOLD_STAND_DIST 以内に迫ったら歩みを止めてその場で構える — 前へ歩き
        // 続けると突進してくるプレッサーとの相対速度(1.8+6.5 m/s)でギャップが倍速で潰れ、ホールドが
        // 半減する(4×5min トレースで確認)。実GKも寄せられたら止まって蹴る。
        const target =
          nearestOpponentDist(world, p) < GK_HOLD_STAND_DIST ? { ...p.pos } : gkReleaseSpot(p);
        p.moveTarget = target;
        p.intent = { kind: 'gkHold', target, until: world.clock + 0.3, possTeam };
        continue;
      }
      // リリース: ホールド state をクリアして配球を決める(整えは挟まない)
      p.gkGainedAt = -1;
      p.intent = null;
      decideOwner(world, p);
      continue;
  ...
  // Task AV: フリーボールの decideDefense 呼び出し順も firstTeam に従う(player loop と対)。
  // owner がいる場合は守備側1チームだけなので順序の概念はなく従来どおり。
  if (owner) {
    decideDefense(world, owner.team === 0 ? 1 : 0);
  } else {
    decideDefense(world, firstTeam);
    decideDefense(world, (1 - firstTeam) as 0 | 1);
  }
}
