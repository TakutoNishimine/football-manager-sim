...
import { laneRiskFromPoints, predictRunUtility, type LaneDefender } from './predict';
...
import { add, dist, len, norm, scale, sub, vec, type Vec } from './vec';
 ...
 */
function passTarget(owner: Player, t: Player): Vec {
...
function interceptInfo(world: World, p: Player): { point: Vec; time: number } {
  ...
  const step = 0.1;
  for (let t = step; t <= 3.0; t += step) {
    pos = add(pos, scale(vel, step));
    vel = scale(vel, Math.exp(-BALL_DAMPING * step));
    if (dist(p.pos, pos) <= reachDist(t)) return { point: pos, time: t };
  }
...
/** ボール保持者の意思決定: シュート / パス / ドリブル を点数で比較 */
function decideOwner(world: World, owner: Player): void {
  ...
  if (owner.instruction?.kind === 'pass') {
    ...
    executePass(world, owner, passTarget(owner, receiver));
    return;
  }
  ...
  if (owner.role === GK_ROLE) {
    ...
    if (best && (pressure < 0.5 || bestPassScore > 0.8)) {
      executePass(world, owner, passTarget(owner, best));
    } else {
      // 安全なパスがない、または詰められている → 最前線の味方方向へクリア
      const fw = teammates(world, owner)
        .filter((t) => t.role !== GK_ROLE)
        .reduce((a, b) => (dist(a.pos, goal) < dist(b.pos, goal) ? a : b));
      executePass(world, owner, add(fw.pos, scale(norm(sub(goal, fw.pos)), 8)));
    }
    return;
  }
  ...
  let bestAction: 'shoot' | 'dribble' | 'hold' | Player = 'dribble';
  let bestScore =
    W.dribbleBase +
  ...
  for (const t of teammates(world, owner)) {
    ...
    const risk = laneRisk(world, owner.pos, target, owner.team);
    if (risk > 0.7) continue; // ほぼ確実にカットされるパスは選ばない
    ...
    // 攻撃参加のラン(レイトラン/オーバーラップ/アンダーラップ)で前へ走っている味方は
    ...
    if (
      ...
      progress > 0 &&
      t.intent &&
      (t.intent.kind === 'lateRun' || t.intent.kind === 'overlap' || t.intent.kind === 'underlap')
    ) {
      score += W.runReceiverBonus;
    }
  ...
  if (pressure < HOLD_MAX_PRESSURE && inAttackingThird) {
    ...
    for (const t of teammates(world, owner)) {
      ...
      if (
        ...
        dist(t.pos, t.intent.target) > 3 // まだ走り込み中(到着していない)
      ) {
        runsMidFlight++;
      }
    }
  ...
  if (bestAction === 'shoot') {
    ...
    executePass(world, owner, passTarget(owner, bestAction));
...
function chooseOffBallIntent(world: World, p: Player, owner: Player): void {
  ...
  // 裏抜け: 相手最終ライン(GK除く)の背後へ走る
  const opps = opponents(world, p.team).filter((o) => o.role !== GK_ROLE);
  const lineX =
    p.team === 0 ? Math.max(...opps.map((o) => o.pos.x)) : Math.min(...opps.map((o) => o.pos.x));
  const behindX = Math.max(-HALF_L + 2, Math.min(HALF_L - 2, lineX + sign * 6));
  for (const dy of [0, -7, 7]) {
    cands.push({
      kind: 'runBehind',
      target: clampToPitch(vec(behindX, p.pos.y + dy)),
      bonus: 0.05,
  ...
  for (const c of cands) {
    const u = c.kind === 'hold' ? baseline : predictRunUtility(world, p, c.target);
...
/** マーク目標点(ゴール側 + ボール寄りシェード)。predict.ts のミラーと揃えること(invariant)。 */
function markTarget(markPos: Vec, ownGoal: Vec, ballPos: Vec, markOffset: number): Vec {
  ...
  return add(goalSide, shade);
}

function decideDefense(world: World, defendersTeam: number): void {
  // GKはプレス・マークに参加しない(aiStep側で専用ポジショニング)
  const defenders = world.players.filter((p) => p.team === defendersTeam && p.role !== GK_ROLE);
  ...
  for (const m of markers) {
    ...
    if (mark) {
      ...
      setIntent(m, 'mark', markTarget(mark.pos, ownGoal, ball.pos, markOffset));
      continue;
    }
  ...
  for (const p of world.players) {
    ...
    if (p.role === GK_ROLE && world.ball.ownerId !== p.id) {
      ...
      if (world.ball.ownerId === null && ballOwnDepth < 22 && dGKBall < 20) {
        const inBox = ballOwnDepth < 11; // ゴール至近では相手より遅くても出る
        const nearestOppToBall = Math.min(
          ...opponents(world, p.team).map((o) => dist(o.pos, world.ball.pos)),
        );
        sweep = inBox || dGKBall < nearestOppToBall + 1.5;
      }
      if (sweep) {
