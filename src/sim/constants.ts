...
// ── GK配球パーソナリティ(Task AB): 受球後のホールド + ショート/ロングの選択 ───────
// 受球した瞬間に配球していた(zero hold)のを、~2〜5秒ためてから配るリズムに変える。
// ためている間はリリース地点(ボックス端方向)へゆっくり歩く。プレッシャーがホールドを
// 滑らかに縮め、相手が詰めると即配球=既存のパントの逃し弁を殺さない。
// seed は world.rng のみ(Math.random/Date 不使用=決定論)。
export const GK_HOLD_MIN = 2; // 無圧時のホールド下限(秒)
export const GK_HOLD_MAX = 5; // 無圧時のホールド上限(秒)
// ホールド短縮の圧力帯: CALM 以下は seed したフル時間を保ち、CALM→CUT の間で線形に0へ縮む。
// CUT で効果時間0=即配球。pressure = 1 - oppDist/4。
// 較正の経緯(Task AB): 初版 CUT=0.5(相手2m)は TACKLE_RANGE(2.2m)の内側で、しかもボールは
// DRIBBLE_OFFSET(0.5m)だけ歩行方向=プレッサー側に置かれるため、放す前にタックルが届く
// (dispos 8%・boxTO 0.5/m, 4×4min 実測)=§4 プレスファームの入口。CUT=0.3(相手2.8m ⇔
// ボール〜2.3m > 2.2m)でタックルが届く前に必ず放す。CALM=0: 相手が4m以内に入った時点から
// 滑らかに縮み始め、経過時間と縮んだ効果時間の交差でリリースが決まる(=長く持っているほど
// 低い圧力で放す。突進してくるプレッサーには3〜3.5m時点で放す挙動になる)。
export const GK_HOLD_CALM = 0;
export const GK_HOLD_PRESSURE_CUT = 0.3;
// ホールド中の歩行は相手がこの距離以内に迫ったら止める(その場で構える)。歩き続けると
// 突進プレッサーとの相対速度でギャップが倍速で潰れ、ホールドが半減する(Task AB トレース)。
export const GK_HOLD_STAND_DIST = 8;
export const GK_WALK_SPEED = 1.8; // ホールド中の歩行上限(m/s)。pace.ts の 'walk' と同値
// 配球の選択(ショート・ビルドアップ vs ロング・パント)。単一のハードな圧力ゲートではなく、
// ショートの質(既存のレーンリスク採点 bestPassScore, クリーンなレーンで ~1.85)と、ロングの
// 価値を突き合わせて選ぶ。ロング価値 = GK_LONG_BASE + GK_LONG_PRESSURE_COEF*pressure。
// クリーンなショートがあり無圧ならショートが勝つ(=ほとんどの局面でショート)。ショートが
// 塞がれている(bestPassScore が低い/皆無)か、詰められている(pressure↑)ときにパントが勝つ。
// coef は掃引不要のため setter を付けない(命名定数として保持)。
export const GK_LONG_BASE = 0.8;
export const GK_LONG_PRESSURE_COEF = 2.2;
...
// カバー判定の速度先読み(Task AF): 押し出しの徒競走の間に走り込んでくるカバーも「カバーされている」
// と読む(位置 + 速度×この秒数でも openBehind を評価し、近い方を採用)。挟み込み(2人目のプレッサー)の
// 導入で「向かってくるカバーへ仕掛ける」非現実な過発火が顕在化したための honest 化(バーストは ~1.1s)。
export const TAKEON_COVER_LOOKAHEAD = 0.8;
...
// シュートの角度誤差スケール(Task AQ、期待値ノブ)。executeShot の実行誤差 errMag に掛ける乗数。
// 1 未満=フィニッシュが正確になり隅を突く=GKを破りやすい(得点↑)、1 超=散る。シュート「選択」
// (decideOwner)は不変なので本数を変えず、変えるのは決定率のみ=純粋な期待値レバー。掃引のため
// setter で上書き可能(setTeamWeights と同型の in-process 設定。env 上書きは存在しない)。
// 出荷値 0.5(Task AQ 較正): AA が得点動作点を 1.525(帯域の床)へ落とした主因はエアリアル動詞に
// よるシュート本数減(-2.0本/tm, run 28892866813)で、動詞を守ったまま期待値側で回復するのが本レバー。
// 40×10 の格子(run 28895164222)で 0.5 が react 0.25/0.31/0.36 を通して goals 2.35-2.45 と最も
// 頑健(GK_REACT_TIME は Z の較正 0.25 を不変のまま)。±感度は reports/task-aq.md 参照。
// Task AD 再較正 0.9: AD がスルーパス/ブレイクの実機構を追加して得点経路を復元したため、
// AQ が「機構不足の補償」として下げた期待値ブーストを中立側へ戻す。40シード×両窓の掃引
// (runs 29047197249 / 29049395716 / 29050866833)で 0.5→3.25/3.52, 0.8→2.98/2.75,
// 0.9→2.55/2.83, 1.0→2.83/2.67(いずれも break-depth 4)— 0.9 がプール平均 2.69 で
// 帯域 2.2-2.8 の中央に最も近い(軸は 0.8 以降飽和し、窓間ノイズ σ≈0.2 が支配的)。
// Task AB 再較正 0.6: AV が確定した帯域割れ(4窓プール goals≈2.05, n=160)の再センター。
// GK配球パーソナリティ(Stage 1)込みの再ベースライン 2.23/1.95・fresh sh/poss 7.7%
// (run 29149363589)から、40シード×両窓の結合掃引(coupled-dials)で 0.7-0.9 は帯域下に
// 平坦(canon 2.10-2.30 / fresh 1.88-2.08; runs 29149806571/29149807638)、二次ダイヤル
// funnel↑・funnel↓・qScale↓ は非改善または片窓が脆い(29149808982/29150770347/29150768652)。
// 0.6 のみが主ダイヤル単独で両窓同時に goals 2.25/2.35・sh/poss 8.2%/8.2% を通す
// (run 29150767108)。感度: 0.55→2.17/2.00, 0.65→2.25/2.38, 0.70→2.23/2.00
// (goals はプラトー、sh/poss ≥8% 両窓は 0.6 のみ)。
export let SHOT_ERR_SCALE = 0.6;
export function setShotErrScale(v: number): void {
  SHOT_ERR_SCALE = v;
}
...
// Task Z 出荷値 0.25(人間の反応窓)。Task Y 中に 0.10 へ下げて goals を帯域に収めたが、0.10 は
// 人間以下の反応窓で「GKは限定的」の実質を損ない Z の較正(0.25 で帯域内)を反転させるため差し戻した。
// Task AQ: 掃引のため setter で実行時に上書きできる(env オーバーライドの tune() ではなく、
// setTeamWeights と同型の決定論的な in-process 設定)。40×10 格子(run 28895164222)で react 軸は
// 浅く(0.25/0.31/0.36 → goals 2.45/2.38/2.35 @ SHOT_ERR_SCALE 0.5)、Z の較正 0.25 を不変で出荷。
export let GK_REACT_TIME = 0.25;
export function setGkReactTime(v: number): void {
  GK_REACT_TIME = v;
}
// GK の浮き球(クロス/パント着地)スイープの慎重さマージン(Task AA)。空中球へは「明確に自分が
// 最初に触れる」= dGK < nearestOpp - この値 のときだけ出る(トラフィックへの飛び出しをためらう実GK
// 挙動)。大きいほど GK がクロスを回収しにくく=ボックスの争いが残り期待値↑。Task AQ で掃引対象に
// するため setter で上書き可能にした(既定 2 は AA の出荷値、magic number を命名定数化)。
// AQ 診断: スイープ全停止(margin 100)でも goals +0.03(run 28892866813)= このマージンは
// 得点の犯人ではない。2 を不変で出荷。
export let GK_AERIAL_SWEEP_MARGIN = 2;
export function setGkAerialSweepMargin(v: number): void {
  GK_AERIAL_SWEEP_MARGIN = v;
}

// キックオフのバックパス窓(Task AT): resetForKickoff からこの秒数の間、中央スポットの持ち手の
// 最初の意思決定を後方/横の最寄り味方への短いパスに誘導する。整え(SETTLE_BASE ~0.63s)を挟んでも
// 最初の判断が確実にこの窓に入るよう、余裕を持って設定する。
export const KICKOFF_PASS_WINDOW = 2.5;
