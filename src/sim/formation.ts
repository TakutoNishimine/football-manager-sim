...
export function dynamicAnchor(world: World, p: Player): Vec {
  ...
  if (!attacking) {
    ...
    const signAY = a.y >= 0 ? 1 : -1;
    const farFactor = Math.max(0, Math.min(1, (-signAY * world.ball.pos.y) / 12));
    y *= 1 - 0.35 * farFactor;

    // DFラインの正規化深さ(GKを除く最小x)と、その撤退後ワールドx。
    // FWの深さ上限と中盤の圧縮基準に使う。
    let dfAX = Infinity;
    for (let i = 0; i < anchors.length; i++) {
      if (i !== GK_ROLE && anchors[i].x < dfAX) dfAX = anchors[i].x;
    }
    const DF_RETREAT = -7; // DFラインの撤退量(最小)。ブロック全体の基準深さ
    const dfWorldX =
      sign * dfAX * HALF_L * 0.78 +
      world.ball.pos.x * 0.25 +
      sign * world.tactics[p.team].lineHeight * 7 +
      sign * DF_RETREAT;
    ...
    if (cls.isFW) {
      ...
      // DFライン(a.x==dfAX)は圧縮ゼロ=撤退のみ → ライン高さ(監視指標)は不変。
      const BLOCK_COMPRESS = 0.25; // 中盤の対DFライン間隔を25%に圧縮(実質 ~5m のバンク)
      const compressedAX = dfAX + (a.x - dfAX) * BLOCK_COMPRESS;
      x += sign * (compressedAX - a.x) * HALF_L * 0.78; // 自基準→圧縮基準へシフト
      x += sign * DF_RETREAT; // ブロック全体の撤退
    }
  } else {
