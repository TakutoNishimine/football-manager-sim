...
/**
 * 守備時にFWが自DFラインより前に出られる上限(m)。ブロックの前後スプレッド(block height)の
 * 上端を握る主要レバー(Task AC で導入、Task AF で較正)。相手ラインにピン留めして間延び
 * するのを防ぎつつ、「出口役のFWは高く残す=カウンターの逃げ場を保つ」ため深くしすぎない
 * (得点床の保護)。掃引用 setter あり(line.ts と同型の in-process 上書き)。
 * Task AF の用量反応(40×10分, cap∈{26,22,20,18,16}, run 29106003356): block 43.95→41.93 と
 * 浅い勾配(~0.2m/m)で 42 付近に飽和。cap=20 が得点 2.60(帯内)・DF-MF 12.65 で最良 —
 * 18/16 は得点が帯を割る(1.77/1.95)ため 20 で確定。
 */
export let FW_CAP_AHEAD = 20;
export function setFwCapAhead(v: number): void {
  FW_CAP_AHEAD = v;
}
