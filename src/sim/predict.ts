...
// プレス目標点・回復ラン・ジョッキー/カバーのジオメトリは press.ts を共有(Task AF)。
 ...
 */
export function laneRiskFromPoints(
  from: Vec,
  to: Vec,
  defenders: LaneDefender[],
  reachRadius: number = BALL_FLIGHT_REACH_RADIUS,
): number {
  ...
  for (const o of defenders) {
    for (let i = 1; i <= 6; i++) {
      ...
      const speed =
        o.engages && dToPoint < reachRadius ? PLAYER_MAX_SPEED : o.speed;
...
export function loftedLaneRiskFromPoints(
  ...
  const defArrive = (point: Vec, o: LaneDefender): number => {
    const dToPoint = dist(o.pos, point);
    const speed = o.engages && dToPoint < reachRadius ? PLAYER_MAX_SPEED : o.speed;
...
export function predictRunUtility(world: World, mover: Player, target: Vec): RunUtility {
  ...
  if (buildup) {
    ...
    if (fws.length) {
      ...
      // Task AF: 出て行く距離は pressIntensity の連続関数(press.ts 共有 = decideDefense と同一)
      if (dist(nearestFw.pos, ballPos) < fwPressRange(tactics.pressIntensity)) fwPresser = nearestFw;
    }
  }
  ...
  // 2人目のプレッサー(Task AF): decideDefense と同じ選定(2番目に近い守備者が
  // secondPressRange 以内。回復中の除外も同一)。目標は press.ts の共有カバー点。
  let secondPresser: Player | null = null;
  {
    ...
    if (rest.length) {
      const cand = rest.reduce((x, y) =>
        dist(x.pos, ballPos) < dist(y.pos, ballPos) ? x : y,
      );
      if (dist(cand.pos, ballPos) < secondPressRange(tactics.pressIntensity)) secondPresser = cand;
    }
  }
  ...
  for (const d of allDefenders) {
    if (d.role === GK_ROLE) {
      ...
      if (d === fwPresser) {
        ...
        // Task AF: decideDefense と同じコミット/ジョッキー分岐(press.ts の共有実装)。
        // hasPressCover は位置の純関数なので同tickで必ず同じ判定になる。press は sprint。
        const rt = hasPressCover(defenders, d, owner, oppSign, tactics.pressIntensity)
          ? pressTarget(owner)
          : jockeyTarget(owner, ownGoal, tactics.pressIntensity);
        pushDef(d, rt, PLAYER_MAX_SPEED, 'press');
      }
    } else if (d === secondPresser) {
      pushDef(d, pressCoverTarget(owner, oppSign, tactics.pressIntensity), PLAYER_MAX_SPEED, 'press');
      ...
      if (mark) {
        used.add(markId);
        pushDef(d, markTargetPoint(mark, ownGoal, ballPos, markOffset, ownLineX, defSign, pressAggroScale(tactics.pressIntensity)), effortSpeed('run'), 'mark');
  ...
  for (const a of attackers) {
    ...
    // Task AF: 守備側の飛行反応半径は pressIntensity の連続関数(現実の world.ts と共有)
    const receivable =
      (1 - laneRiskFromPoints(ballPos, ap, defLane, flightReachRadius(tactics.pressIntensity))) *
      Math.min(1, minDef / 6);
