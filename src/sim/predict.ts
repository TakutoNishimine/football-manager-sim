...
/** ai.ts の markTarget のミラー(invariant)。ゴール側 + ボール寄りシェード。 */
function markTargetPoint(markPos: Vec, ownGoal: Vec, ballPos: Vec, markOffset: number): Vec {
...
const DEFENDER_REACTION = 0.35; // パスを見てから動き出すまでの反応時間
...
export function predictRunUtility(world: World, mover: Player, target: Vec): RunUtility {
  ...
  for (const d of allDefenders) {
    if (d.role === GK_ROLE) {
      ...
      if (mark) {
        used.add(markId);
        pushDef(d, markTargetPoint(mark, ownGoal, ballPos, markOffset), effortSpeed('run'), 'mark');
