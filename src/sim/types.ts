...
export type IntentKind =
  ...
  | 'gkHold'; // GK: 受球後、配球までボールを持って(ゆっくり歩いて)ためる(Task AB)
...
export interface Player {
  ...
  // ── GK配球パーソナリティ(Task AB) ────────────────────────────────────
  /**
   * GKがこのボールを保持し始めた試合時刻(受球のたびに1回だけ seed)。-1 = 保持していない
   * (=次に受球したら再 seed)。off-ball の keeper ポジショニングに入るたびに -1 へ戻す。
   */
  gkGainedAt: number;
  /** 今回の保持で seed した無圧時のホールド目標時間(秒, GK_HOLD_MIN〜GK_HOLD_MAX)。圧力で縮む */
  gkHoldBase: number;
}
