...
export const DEFAULT_WEIGHTS: AiWeights = {
  ...
  dribbleBase: 0.55,
  dribbleCalm: 0.5,
  // Task AT 再較正 0.28→0.50: Task AD のスルーパス動詞とフラットなラインが仕掛けの局面を
  // 奪い(6.99→4.29/tm, 実証済みの置換)、ゲート(6-16 @ 35-60%)を割った。フル連成掃引
  // (duel+funnel+goals両窓+offside を1コンフィグずつ同時計測, runs 29073396869 / 29076154238 /
  // 29076155523)で 0.50/0.60 が仕掛け 10.24/11.29 @ 41.1/43.0%(canon/fresh)、goals 2.58/2.33
  // で両窓帯域内。±0.05 感度: 0.45→9-10本・goals 3.00/2.70、0.55→12本・goals 2.80/2.38
  // (窓間ノイズ σ≈0.2 内)。0.75 系は goals 2.90-2.92 で過熱、space 1.6 系は仕掛け 16-18本で
  // 上限超え。takeOnSpace は 0.6 を不変(1.1 でも通るが poss/min が許容縁 7.27-7.28 に張り付く)。
  takeOnBase: 0.5,
  takeOnSpace: 0.6,
  crossBase: 2.2,
  crossBodies: 0.5,
